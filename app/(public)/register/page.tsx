...
export default function Page() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, reset } =
    useForm<RegisterVendorInput>({ resolver: zodResolver(RegisterVendorSchema),
      defaultValues: {
  ...
  async function onSubmit(values: RegisterVendorInput) {
    ...
    // 1) Loguear automáticamente con NextAuth
    ...
    if (signed?.error) {
      setServerError("Usuario creado pero no se pudo iniciar sesión.");
      return;
    }

    reset();

    // 2) Redirigir al flujo de OAuth de Mercado Pago (sesión ya activa)
